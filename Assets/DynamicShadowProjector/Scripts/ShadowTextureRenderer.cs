...
#pragma warning restore // CS1692, RECS0001
        ...
        static void StaticInitialize()
        {
			...
			s_shadowColorParamID = Shader.PropertyToID("_Color");
		...
		void AddPostRenderPassCommands(CommandBuffer cmd, RenderTargetIdentifier srcId)
		{
			...
			else
			{
				if (m_superSampling != TextureSuperSample.x1 || HasShadowColor())
				{
					...
					{
						++pass;
						if (downsampleProperties == null)
						{
							downsampleProperties = AllocatePropertyBlock();
						}
						downsampleProperties.SetColor(s_shadowColorParamID, m_shadowColor);
					}
					cmd.SetRenderTarget(dstId);
					AddBlitToCurrentTargetCommand(cmd, srcId, m_downsampleShader, pass, downsampleProperties);
					tempRTIndex ^= 1;
